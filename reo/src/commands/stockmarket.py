...
class StockMarket(commands.Cog):
    ...
    @commands.group(name="stock", invoke_without_command=True)
    async def stock(self, ctx):
        """Stock Market Commands"""
        await ctx.send_help(ctx.command)

    @stock.command(name="market")
    async def market(self, ctx):
        """View the stock market overview"""
        embed = discord.Embed(
    ...
    @stock.command(name="leaderboard")
    ...
    @stock.command(name="transactions")
    ...
    @commands.group(name="stockadmin", invoke_without_command=True)
    async def stockadmin(self, ctx):
        """Admin-only stock market commands"""
        if not await self.bot.is_owner(ctx.author):
            return await ctx.send("❌ Owner only!")
        await ctx.send_help(ctx.command)

    @stockadmin.command(name="setprice")
    ...
    @stockadmin.command(name="resetuser")
