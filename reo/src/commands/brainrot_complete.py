...
class Brainrot(commands.Cog):
    ...
    async def get_brainrot_definition(self, guild_id):
        ...
        name, rarity = random.choice(DEFAULT_BRAINROTS)
    ...
    async def capture_spawn(self, guild_id, user_id, spawn_id):
        ...
        if not spawn or spawn["id"] != spawn_id:
    ...
    async def spawn_loop(self):
        for guild in self.bot.guilds:
            try:
                ...
                if guild.id in self.active_spawns:
                    continue
...
`!brainrot setup` — Owner: create the 3 Brainrot channels
