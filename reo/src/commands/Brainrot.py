...
DEFAULT_BRAINROTS = [
    ...
    ("Chimpanzini Bananini", "Uncommon"),
    ("Bobrito Bandito", "Uncommon"),
    ("Frulli Frulla", "Uncommon"),
    ("Orangutini Ananasini", "Rare"),
    ...
    ("Svinino Bombondino", "Secret"),
    ("Pot Hotspot", "Secret"),
    ("Espresso Signora", "Secret"),
    ("Secret Lucky Block", "Secret"),
    ("Dragon Cannelloni", "Secret"),
    ("Elephantuchi Bananuchi", "Legendary"),
    ("Penguino Cocosino", "Epic"),
    ("Zibra Zubra Zibralini", "Epic"),
    ("Rhino Helicopterino", "Rare"),
    ("Spioniro Golubiro", "Rare"),
    ("Ballerino Lololo", "Rare"),
    ("Crocodillo Ananasini", "Uncommon"),
    ("Gato Celesto", "Uncommon"),
    ("Pipi Kiwi", "Common"),
    ("Ta Ta Ta Sahur", "Common"),
    ("Boneca Ambalabu", "Common"),
    ("Capuccino Clownino", "Uncommon"),
    ("Makakini Bananini", "Rare"),
    ("Cacto Hipopotamo", "Epic"),
...
class Brainrot(commands.Cog):
    ...
    async def get_brainrot_definition(self, guild_id):
        ...
        # Once defaults are seeded into custom_brainrots, the database
        # becomes the editable active pool for this guild.
        if custom:
            if lucky:
                boosted = [x for x in custom if x["rarity"] in {"Epic", "Legendary", "Mythic", "Secret"}]
                if boosted and random.random() < 0.45:
    ...
    async def brainrot_help(self, ctx, action: Optional[str] = None, *args):
        ...
        if action == "setup":
            ...
            await self.db.execute(
                """INSERT INTO settings(guild_id,summon_channel,market_channel,base_channel,admin_channel,interval)
                   VALUES(?,?,?,?,?,120)
            ...
            # Seed the 50 editable default Brainrots for this guild.
            # Existing customizations are preserved.
            for name, rarity in DEFAULT_BRAINROTS:
                data = RARITIES[rarity]
                await self.db.execute(
                    """INSERT OR IGNORE INTO custom_brainrots
                       (guild_id,name,rarity,value,income,image)
                       VALUES(?,?,?,?,?,?)""",
                    (guild.id, name, rarity, data["value"], data["income"], None),
                )
...
"""
    ...
    async def owner_edit(self, ctx, name: str, field: str, *, value: str):
        ...
        if not row:
            ...
            data = RARITIES[r]
            await self.db.execute(
                "INSERT INTO custom_brainrots(guild_id,name,rarity,value,income,image) VALUES(?,?,?,?,?,?)",
                (ctx.guild.id, n, r, data["value"], data["income"], None),
            )
            row = await self.db.fetchone(
                "SELECT * FROM custom_brainrots WHERE guild_id=? AND name=?",
                (ctx.guild.id, n),
        ...
        if field == "name":
            ...
            if not new_name:
                return await ctx.send("❌ Name cannot be empty.")
            exists = await self.db.fetchone(
                "SELECT 1 FROM custom_brainrots WHERE guild_id=? AND name=?",
                (ctx.guild.id, new_name),
            )
            if exists:
                return await ctx.send("❌ That name already exists.")
            await self.db.execute(
                "UPDATE custom_brainrots SET name=? WHERE guild_id=? AND name=?",
