...
DEFAULT_BRAINROTS = [
    ...
    ("Frulli Frulla", "Common"),
    ("Trippi Troppi", "Common"),
    ("Chimpanzini Bananini", "Common"),
    ("Frigo Camelo", "Common"),
    ("Brr Brr Patapim", "Uncommon"),
    ("Bombardiro Crocodilo", "Uncommon"),
    ("Bombombini Gusini", "Uncommon"),
    ("Bananita Dolphinita", "Uncommon"),
    ("Cocofanto Elefanto", "Uncommon"),
    ("Boneca Ambalabu", "Uncommon"),
    ("Bobrito Bandito", "Uncommon"),
    ("Orangutini Ananasini", "Uncommon"),
    ("Ballerina Cappuccina", "Rare"),
    ("Cappuccino Assassino", "Rare"),
    ("Brri Brri Bicus Dicus Bombicus", "Rare"),
    ("La Grande Combinasion", "Rare"),
    ("Gorillo Watermellondrillo", "Rare"),
    ("Glorbo Fruttodrillo", "Rare"),
    ("Ta Ta Ta Ta Sahur", "Rare"),
    ("Trulimero Trulicina", "Rare"),
    ("Burbaloni Loliloli", "Epic"),
    ("Talpa Di Ferro", "Epic"),
    ("Blueberrinni Octopussini", "Epic"),
    ("Cavallo Virtuoso", "Epic"),
    ("Zibra Zubra Zibralini", "Epic"),
    ("Tigroligre Frutonni", "Epic"),
    ("Rhino Toasterino", "Epic"),
    ("Avocadini Antilopini", "Epic"),
    ("Pipi Kiwi", "Legendary"),
    ("La Vaca Saturno Saturnita", "Legendary"),
    ("Graipuss Medussi", "Legendary"),
    ("Tralalero Tralalita", "Legendary"),
    ("Espressona Signora", "Legendary"),
    ("Matteo", "Legendary"),
    ("Los Tralaleritos", "Legendary"),
    ("Nuclearo Dinossauro", "Mythic"),
    ("Cocofanto Elefanto Supremo", "Mythic"),
    ("Pot Hotspot", "Mythic"),
    ("Dragon Cannelloni", "Mythic"),
    ("Ketupat Kepat", "Mythic"),
    ("Chicleteira Bicicleteira", "Mythic"),
    ("Secret Lucky Block", "Secret"),
    ("Strawberrelli Flamingelli", "Secret"),
    ("Garamararamararaman dan Madudungdung Tak Tudung", "Secret"),
    ("La Esok Sekolah", "Secret"),
    ("Brainrot Overlord", "Secret"),
]
...
class Brainrot(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self._last_spawn = {}
        self._spawn_loop_started = False
    ...
    async def spawn_loop(self):
        for guild in self.bot.guilds:
            try:
                ...
                interval = max(10, int(s["interval"] or DEFAULT_SPAWN_SECONDS))
                now = time.time()
...
"""
    ...
    @owner.command(name="edit", aliases=["update"])
    async def owner_edit(self, ctx, name: str, field: str, *, value: str):
        ...
        if not row:
            ...
            if found:
                n, r = found
                await self.db.execute(
                    "INSERT OR IGNORE INTO custom_brainrots(guild_id,name,rarity,value,income,image) VALUES(?,?,?,?,?,?)",
                    (ctx.guild.id, n, r, RARITIES[r]["value"], RARITIES[r]["income"], ""),
                )
                row = await self.db.fetchone(
                    "SELECT * FROM custom_brainrots WHERE guild_id=? AND name=?",
                    (ctx.guild.id, n),
        ...
        shown_name = value if field == "name" else name
        await ctx.send(f"✅ Updated **{name}** → `{field}={value}`. Current name: **{shown_name}**")
    ...
    async def owner_enable(self, ctx):
        if not self.is_owner(ctx.author.id):
            return await ctx.send("❌ Bot owner only.")
        await self.db.execute(
            "UPDATE settings SET enabled=1, interval=CASE WHEN interval IS NULL OR interval < 10 THEN ? ELSE interval END WHERE guild_id=?",
            (DEFAULT_SPAWN_SECONDS, ctx.guild.id),
        )
        self._last_spawn[ctx.guild.id] = time.time()
        await ctx.send(f"✅ Brainrot auto-spawn enabled. Next spawn in about **{int((await self.settings(ctx.guild.id))["interval"]):,} seconds**.")
...
async def setup(bot: commands.Bot):
    ...
    if not cog.spawn_loop.is_running():
        cog.spawn_loop.start()
        cog._spawn_loop_started = True
