...
"""
    ...
    @brainrot_help.command(name="debug")
...
Summon Channel ID: `{s['summon_channel']}`
Enabled: `{s['enabled']}`
Interval: `{s['interval']}s`
Loop Running: `{self.spawn_loop.is_running()}`
Last Spawn: `{self._last_spawn.get(ctx.guild.id, 'Never')}`
        """)

    @brainrot_help.command(name="startloop")
    async def start_loop(self, ctx):
        if not self.is_owner(ctx.author.id):
            return await ctx.send("❌ Bot owner only.")
        
        if self.spawn_loop.is_running():
            return await ctx.send("✅ Loop is already running!")
        
        self.spawn_loop.start()
        await ctx.send("✅ Spawn loop started! Brainrots should spawn now.")
