...
class YouTube(commands.Cog):
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params=params) as response:
                    if response.status != 200:
                        ...
                        logger.warning(
                            f"YouTube API {response.status}: {body[:300]}"
                        )
                        return None
    ...
    async def resolve_channel(self, value):
        ...
        if re.fullmatch(r"UC[A-Za-z0-9_-]{20,}", value):
            ...
            match = re.search(
                r"youtube\.com/channel/(UC[A-Za-z0-9_-]+)", value
            )
            if match:
        ...
        if value.startswith("@"):
            ...
            match = re.search(r"youtube\.com/@([^/?&]+)", value)
            if match:
    ...
    async def _search(self, interaction, query):
        ...
        if data is None:
            return await interaction.followup.send(
                "❌ YouTube API key missing or API request failed."
            )
    ...
    @youtube.command(name="search", description="Search YouTube videos")
    @app_commands.describe(query="What do you want to search?")
    async def youtube_search(self, interaction, query: str):
        await self._search(interaction, query)

    @yt.command(name="search", description="Search YouTube videos")
    @app_commands.describe(query="What do you want to search?")
    async def yt_search(self, interaction, query: str):
        await self._search(interaction, query)
    ...
    async def _video(self, interaction, value):
        ...
        embed = discord.Embed(
            title=self.trim(snippet.get("title", "YouTube Video"), 256),
            description=self.trim(snippet.get("description", ""), 1000),
            url=url,
            color=discord.Color.red(),
        )
        ...
        embed.add_field(
            name="👤 Channel",
            value=snippet.get("channelTitle", "N/A"),
            inline=True,
        )
        embed.add_field(
            name="👁️ Views",
            value=self.format_number(stats.get("viewCount")),
            inline=True,
        )
        embed.add_field(
            name="👍 Likes",
            value=self.format_number(stats.get("likeCount")),
            inline=True,
        )
        embed.add_field(
            name="💬 Comments",
            value=self.format_number(stats.get("commentCount")),
            inline=True,
        )
        embed.add_field(
            name="📅 Published",
            value=snippet.get("publishedAt", "N/A")
            .replace("T", " ")
            .replace("Z", " UTC"),
            inline=True,
        )
    ...
    @youtube.command(name="video", description="Show YouTube video details")
    @app_commands.describe(video="YouTube URL, video ID, or search text")
    async def youtube_video(self, interaction, video: str):
        await self._video(interaction, video)

    @yt.command(name="video", description="Show YouTube video details")
    @app_commands.describe(video="YouTube URL, video ID, or search text")
    async def yt_video(self, interaction, video: str):
        await self._video(interaction, video)
    ...
    async def _channel(self, interaction, value):
        ...
        embed.add_field(
            name="👥 Subscribers",
            value=self.format_number(stats.get("subscriberCount")),
            inline=True,
        )
        embed.add_field(
            name="👁️ Total Views",
        ...
        embed.add_field(
            name="🎬 Videos",
            value=self.format_number(stats.get("videoCount")),
            inline=True,
        )
        embed.add_field(
            name="🔗 Channel",
    ...
    async def youtube_channel(self, interaction, channel: str):
        await self._channel(interaction, channel)
    ...
    async def yt_channel(self, interaction, channel: str):
        await self._channel(interaction, channel)
    ...
    async def _live(self, interaction, value):
        ...
        live = await self.find_live(channel["id"])

        if not live:
            return await interaction.followup.send(
                f"⚫ **{channel['snippet']['title']}** abhi live nahi hai."
            )
        ...
        thumbnail = live["snippet"].get("thumbnails", {}).get("high")
        ...
        embed.set_footer(text=channel["snippet"]["title"])
    ...
    async def youtube_live(self, interaction, channel: str):
        await self._live(interaction, channel)
    ...
    async def yt_live(self, interaction, channel: str):
        await self._live(interaction, channel)
    ...
    async def _unsubscribe(self, interaction, channel_value):
        ...
        await storage.youtube.delete(id=existing["id"])
    ...
    @app_commands.checks.has_permissions(manage_guild=True)
    @app_commands.describe(channel="YouTube channel")
    async def youtube_unsubscribe(self, interaction, channel: str):
        await self._unsubscribe(interaction, channel)
    ...
    @app_commands.checks.has_permissions(manage_guild=True)
    @app_commands.describe(channel="YouTube channel")
    async def yt_unsubscribe(self, interaction, channel: str):
        await self._unsubscribe(interaction, channel)
    ...
    async def _subscriptions(self, interaction):
        ...
        for row in rows:
            ...
            role = (
                interaction.guild.get_role(row["role_id"])
                if row.get("role_id")
                else None
            )
    ...
    async def youtube_subscriptions(self, interaction):
    ...
    async def yt_subscriptions(self, interaction):
        await self._subscriptions(interaction)
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    live = await self.find_live(row["channel_id"])
                    ...
                    if row.get("last_live_video_id") == video_id:
                        continue
                    ...
                    embed.set_footer(
                        text=f"{row['channel_name']} • YouTube Live"
                    )
